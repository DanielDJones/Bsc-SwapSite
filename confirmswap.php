...
//FIND LISTING CREATOR ID
$getCreatorID = $mysqli->query("SELECT * FROM LISTING WHERE LISTINGID= $listingID ") or die($mysqli->error());
$getCreatorID2 = $getCreatorID->fetch_assoc();

$accountID = $getCreatorID2['ACCOUNTID'];
$custID = $offer['CUSTID'];
...
{
  $sql = "UPDATE OFFER SET OFFERACCEPTED = '2' WHERE OFFER.OFFERID = $offerID ";
  if ( $mysqli->query($sql) ){
    $sql = "UPDATE LISTING, OFFER SET LISTINGACTIVE = '0' WHERE OFFER.OFFERID = $offerID ";
    if ( $mysqli->query($sql) ){
        header("location: confirmswap.php?id=$offerID");
    }
    else {
...
$getCustID = $mysqli->query("SELECT * FROM Cust WHERE CUSTID='$custID'");
$getCUSTID2 = $getCustID->fetch_assoc();
$custAcountID = $getCUSTID2['ACCOUNTID'];

$result2 = $mysqli->query("SELECT * FROM Account WHERE ACCOUNTID= $custAcountID ") or die($mysqli->error());
...
$result4 = $mysqli->query("SELECT * FROM LISTING WHERE LISTINGID= $listingID ") or die($mysqli->error());
...
$offerAccountUsername = $offerAccount['USERNAME'];
